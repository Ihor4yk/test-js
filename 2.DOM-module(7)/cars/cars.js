...
function createMarkup(arr) {
  return arr.map(({ id, car, type, price, img }) => `
    <li class="car-card" data-id="${id}">
      <img class="car-image" src="${img}" alt="${type}">
      <div class="container">
...
function handleSubmit(event) {
  ...
  const queryValue = formElements.query.value;
  const optionsValue = formElements.options.value;
  // console.log(queryValue);
  // console.log(optionsValue);

  const result = cars.filter((car) =>
    car[optionsValue].toLowerCase() === queryValue.toLowerCase());
  // console.log(result);
